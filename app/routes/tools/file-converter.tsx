    ...
    [
      ...
      },
      {
        "script:ld+json": course({
          ...
          description:
            "Learn how to convert images and PDF documents between formats securely in your browser",
        }),
      },
...
export default function FileConverter({loaderData}: any) {
              ...
              <div className="space-y-6">
                <div className="flex flex-col md:flex-row items-stretch gap-4 p-6 bg-slate-50 dark:bg-slate-800/30 rounded-2xl border border-slate-200 dark:border-slate-700">
                  ...
                  <div className="flex-1 space-y-2">
                    ...
                    >
                      <option value="">
                        {t("file.convert.select_format")}
                      </option>
                      {sourceType &&
                        CONVERSIONS[sourceType]?.map((ext) => (
                ...
                <div className="space-y-2 max-h-96 overflow-y-auto pr-2 custom-scrollbar">
                  {files.map((f, i) => (
                    <div
                      key={i}
                      className="flex items-center gap-4 p-4 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl animate-in fade-in slide-in-from-left-2 duration-300"
        ...
        <section className="grid grid-cols-1 md:grid-cols-3 gap-8 py-12">
          <div className="p-8 bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm hover:shadow-md transition-shadow">
            ...
            <p className="text-slate-600 dark:text-slate-400 leading-relaxed">
              No servers involved. All file conversions happen directly in your
              browser using the latest technologies. Your data is 100% safe.
            </p>
          </div>
          <div className="p-8 bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm hover:shadow-md transition-shadow">
            ...
            <p className="text-slate-600 dark:text-slate-400 leading-relaxed">
              Convert multiple files simultaneously. High-performance
              browser-based processing makes it fast and efficient for any
              volume.
            </p>
          </div>
          <div className="p-8 bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm hover:shadow-md transition-shadow">
            ...
            <p className="text-slate-600 dark:text-slate-400 leading-relaxed">
              Convert between all major image formats (PNG, JPG, WEBP, SVG) and
              PDF documents with ease. One tool for everything.
            </p>
          </div>
        </section>
        ...
        <section className="bg-slate-50 dark:bg-slate-800/30 rounded-3xl border border-slate-200 dark:border-slate-800 overflow-hidden">
          ...
          <div className="p-8 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
            {Object.entries(CONVERSIONS).map(([type, targets]) => (
              <div
                key={type}
                className="flex flex-col gap-2 p-4 bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800"
              >
                ...
                <div className="flex flex-wrap gap-2">
                  {targets.map((t) => (
                    <span
                      key={t}
                      className="px-2 py-1 bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 text-[10px] font-bold rounded-lg border border-blue-100 dark:border-blue-900/50 uppercase tracking-tight"
                    >
                      to {t}
                    </span>
                  ))}
