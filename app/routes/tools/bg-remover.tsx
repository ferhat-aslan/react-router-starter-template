  ...
  return generateMeta(
    {
      ...
      image: `https://kleinbyte.com/${locale === "en" ? "" : locale + "/"}og/tools/bg-remover.png`,
    },
    [
      {
      ...
      },
      {
        "script:ld+json": course({
          ...
          description:
            "Learn how to remove backgrounds from any image using high-quality AI processing directly in your browser.",
        }),
      },
...
export default function BgRemover({loaderData}: any) {
  ...
  const processImage = async () => {
    ...
    try {
      ...
      const {pipeline, env, RawImage} = transformers as any;
      ...
      const segmenter = await pipeline("image-segmentation", model_id, {
        progress_callback: (p: any) => {
          if (p.status === "progress") {
            ...
            setLoadingText(
              `${t("bg.remover.status.loading_model")} ${Math.round(
                p.progress,
              )}%`,
            );
          }
      ...
      maskData.current = ctx.getImageData(
        0,
        0,
        canvas.width,
        canvas.height,
            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                <div className="flex items-center justify-center gap-2 mt-6">
                  <kbd className="px-3 py-1.5 bg-slate-100 dark:bg-slate-800 rounded-lg text-xs font-bold border border-slate-200 dark:border-slate-700">
                    Ctrl + V
                  </kbd>
                  <span className="text-sm text-slate-400 font-medium">
                    {t("bg.remover.paste_hint")}
                  </span>
                </div>
              </div>
            </div>
                  ...
                  {isProcessing && (
                    <div className="space-y-4">
                      <div className="flex justify-between items-end mb-2">
                        <span className="text-sm font-bold text-indigo-500 lowercase">
                          {loadingText}
                        </span>
        ...
        <section className="grid grid-cols-1 md:grid-cols-3 gap-8 py-12">
          <div className="p-8 bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm">
            ...
            <p className="text-slate-600 dark:text-slate-400 leading-relaxed">
              Processed 100% in your browser. No server uploads, no data
              storage. Your photos stay strictly on your device.
            </p>
          </div>
          <div className="p-8 bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm">
            ...
            <p className="text-slate-600 dark:text-slate-400 leading-relaxed">
              Powered by state-of-the-art segmentation models (RMBG-1.4) to
              precisely separate subjects from complex backgrounds.
            </p>
          </div>
          <div className="p-8 bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm">
            ...
            <p className="text-slate-600 dark:text-slate-400 leading-relaxed">
              Export high-resolution PNGs with perfect transparency. Ideal for
              e-commerce, design, and personal projects.
            </p>
          </div>
        </section>
      </div>
